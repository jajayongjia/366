...
def env_init():
    ...
    global exceptQ
    exceptQ = np.random.normal(0,1,10) 
    local_observation = np.zeros(0) # An empty NumPy array

    this_reward_observation = (exceptQ, local_observation, False)
...
def env_step(this_action): # returns (floating point, NumPy array, Boolean), this_action: NumPy array
    global this_reward_observation
    global exceptQ
    meanQ = exceptQ[int(this_action)]
    the_reward = rand_norm(meanQ, 1.0) # rewards drawn from (0, 1) Gaussian
